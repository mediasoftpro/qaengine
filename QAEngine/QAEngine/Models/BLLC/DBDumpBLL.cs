    ...
    public class DBDumpBLL
    {
        ...
        public static async Task InitializeMailTemplates(ApplicationDbContext context)
        {
            if (await MailTemplateBLL.Count(context, new MailTemplateEntity() { type = "-1" }) == 0)
            {
                ...
                if (Settings.Configs.FeatureSettings.enable_qa)
                {
                    MailTemplates.Add(new JGN_MailTemplates
                    {
                        templatekey = "ASKQA",
                        description = "QA - Ask new Question",
                        type = "qa",
                        subjecttags = "[username]",
                    ...
                    {
                        templatekey = "QAANSP",
                        description = "QA - Answer Posted",
                        type = "qa",
                        subjecttags = "[username]",
                        tags = "[pusername],[ausername],[qtitle],[answer],[url]",
                        subject = "New Question Reply on [website]",
                        contents = "<p>Hello [ausername],<br /><br />[pusername] posted answer on your question on <a href='[website_url]' title='[website]'>[website]</a>.</p>" +
                         "<p>[url]</p>" +
                         "<p>[qtitle]</p>" +
                         return_template_signature()
                    });
