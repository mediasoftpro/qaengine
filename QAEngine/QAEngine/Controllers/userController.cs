    ...
    public class userController : Controller
    {
        ...
        public async Task<IActionResult> Index(string username, string status = null, string uid = null)
        {
            ...
            if (userInfo.Count > 0)
            {
                ...
                model.FullName = UserUrlConfig.PrepareUserName(model.user, Configs.RegistrationSettings.uniqueFieldOption);

                model.qa = new qaListViewModel()
                {
                    HeadingTitle = model.FullName + " " + SiteConfig.generalLocalizer["_asked_questions"].Value,
                    ...
                    BrowseUrl = Config.GetUrl("user/qa/" + model.UserName),
                ...
                ViewBag.title= model.FullName + "'s " + SiteConfig.generalLocalizer["_profile"];
        ...
        public async Task<IActionResult> profile(string username)
        {
            ...
            ViewBag.title = model.FullName + "'s " + SiteConfig.generalLocalizer["_profile_detail"];

            return View(model);
        }

        public async Task<IActionResult> qa(string username, int? pagenumber)
        {
            if (pagenumber == null)
                pagenumber = 1;
            ...
            if (userInfo.Count > 0)
            {
                ...
                var _title = model.FullName + "'s " + SiteConfig.generalLocalizer["_asked_questions"] + " (0)"; // " + _list[0].stat_qa + "
                ...
                model.qa = new qaListViewModel()
                {
                    ...
                    {},
                    HeadingTitle = _title,
                    DefaultUrl = Config.GetUrl("user/qa/" + model.UserName),
                    PaginationUrl = Config.GetUrl("user/qa/" + model.UserName + "/[p]/"),
                    NoRecordFoundText = SiteConfig.generalLocalizer["_no_records"].Value,
                };
                model.qa.TotalRecords = await Jugnoon.qa.QABLL.Count(_context, model.qa.QueryOptions);
                if (model.qa.TotalRecords > 0)
                {
                    model.qa.DataList = await Jugnoon.qa.QABLL.LoadItems(_context, model.qa.QueryOptions);
                }
