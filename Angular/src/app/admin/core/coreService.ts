...
export class CoreService {
  ...
  updateCategories(controls: any, categories: any, key = 'categories') {
  ...
  // array = list array
  prepareSelectedItems_v2(selected_list = '', list = []) {
  ...
  /*              map saved data with available dynamic attributes              */
  /* -------------------------------------------------------------------------- */
  prepareDynamicControlData_RealEstate(info: any) {
    /* extend dynamic field binding with different types of properties */
    // homes
    if (info.options_homes.length > 0) {
      for (let option of info.options_homes) {
        for (let attr of option.attributes) {
          for (let value of info.attr_values) {
            if (value.attr_id === attr.id) {
              attr.value = value.value;
    ...
    // plots
    if (info.options_plots.length > 0) {
      for (let option of info.options_plots) {
        for (let attr of option.attributes) {
          for (let value of info.attr_values) {
            if (value.attr_id === attr.id) {
              attr.value = value.value;
    ...
    // commercial
    if (info.options_commercial.length > 0) {
      for (let option of info.options_commercial) {
        for (let attr of option.attributes) {
          for (let value of info.attr_values) {
            if (value.attr_id === attr.id) {
              attr.value = value.value;
  ...
  processDynamicControlsData_RealEstate(payload: any, info: any) {
    const arr = [];
    for (const prop of Object.keys(payload)) {
      if (prop.includes('attr_')) {
        const id = parseInt(prop.replace('attr_', ''), 10);
        ...
        if (info.attr_values.length > 0) {
          ...
          // insert case extend to three sections
          // homes
          for (const option of info.options_homes) {
            for (const attr of option.attributes) {
              if (attr.id === id) {
                obj.attr_id = id;
                obj.title = attr.title;
          ...
          // plots
          for (const option of info.options_plots) {
            for (const attr of option.attributes) {
              if (attr.id === id) {
                obj.attr_id = id;
                obj.title = attr.title;
          ...
          // commercial
          for (const option of info.options_commercial) {
            for (const attr of option.attributes) {
              if (attr.id === id) {
                obj.attr_id = id;
                obj.title = attr.title;
  ...
  /*                             attach encrypted id                            */
  /* -------------------------------------------------------------------------- */
  initializeChartData(dataTable: any, chartType: number) {
    let type = "ColumnChart";
    switch (chartType) {
      case 1:
        type = "BarChart";
        break;
    ...
    return {
      ...
      dataTable: dataTable,
      options: {
        title: "Ads posted in last one year",
        legend: "bottom",
        width: 550,
        height: 350,
        is3D: true
