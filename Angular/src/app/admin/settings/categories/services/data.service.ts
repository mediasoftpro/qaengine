...
import { loadStarted, loadSucceeded, loadFailed, loaddropdowncategories, applyChanges} from "../../../../reducers/settings/categories/actions";
...
export class DataService {
  ...
  LoadDropdownCategories(type: number) {
    ...
    this.http.post(URL, { type }).subscribe(
      (data: any) => {
        // update core data
        this._store.dispatch(new loaddropdowncategories(data));
      },
      err => {
        this._store.dispatch(new loadFailed(err));
      }
