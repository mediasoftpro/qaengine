...
@Component({
  ...
  selector: "app-blog-process",
  animations: [fadeInAnimation],
  providers: [CategoryDataService, CategorySettingService]
})
export class BlogProcComponent implements OnInit {
  constructor(
  ...
  ngOnInit() {
    ...
    this.categories$.subscribe((categories: any) => {
      if (categories.length > 0) {
        ...
        this.categoryDataService.LoadDropdownCategories(6); // 6: blogs
      }
    });
      ...
      (data: any) => {
        if (data.status === "error") {
          ...
          if (!this.isAdmin) {
            ...
            this.router.navigate([this.route_path + "profile/" + data.record.id]);
          }
